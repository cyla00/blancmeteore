...
const continueOrder = async () => {
    const order_data = JSON.parse(localStorage.getItem('site-ecom'))
    ...
    const body = {
        ...
        questNumCategories: order_data.n_categories,
        questNumProducts: order_data.n_products,
        ...
        if(res.status === 200){
            await axios.post('http://localhost:3000/api/create-site-ecommerce', body, auth).then((res) => {
                if(res.status === 200){
                    ...
                    localStorage.removeItem('site-ecom')
                    setTimeout(() => {
...
const removeOrder = () => {
    if(confirm('remove order?')){
        ...
        return localStorage.removeItem('site-ecom')
    }
    return
}

onMounted(() => {
    if(localStorage.getItem('site-ecom')){
