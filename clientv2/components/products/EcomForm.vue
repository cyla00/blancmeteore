...
const n_categories = ref<string>('')
const n_products = ref<string>('')
...
const submit = async () => {
    if(id_graph.value === '' || n_categories.value === '' || n_products.value === ''){
        show.value = true
        return errMsg.value = emptyMsg.value
    }
    ...
    const body = {
        ...
        questNumCategories: n_categories.value,
        questNumProducts: n_products.value,
