...
orderInProgress.post('/api/order-in-progress', async (ctx) => {
    ...
    if(findgraph){
        try{
            await idGraph.updateOne({id: bodyVal.order_id}, {$set: {status: prod_json.status[1], modifiedAt: `${date.getDate()}/${date.getMonth() + 1}/${date.getFullYear()} ${date.getHours()}:${date.getMinutes()}`}})
    ...
    if(findsitevitr){
        try{
            await sitesVitr.updateOne({id: bodyVal.order_id}, {$set: {status: prod_json.status[1], modifiedAt: `${date.getDate()}/${date.getMonth() + 1}/${date.getFullYear()} ${date.getHours()}:${date.getMinutes()}`}})
            ctx.response.status = Status.OK
            return ctx.response.body = {
                SuccMsg: 'Statut changé avec succès'
            }
    ...
    if(findsiteecomm){
        try{
            await sitesEcom.updateOne({id: bodyVal.order_id}, {$set: {status: prod_json.status[1], modifiedAt: `${date.getDate()}/${date.getMonth() + 1}/${date.getFullYear()} ${date.getHours()}:${date.getMinutes()}`}})
            ctx.response.status = Status.OK
            return ctx.response.body = {
                SuccMsg: 'Statut changé avec succès'
            }
    ...
    if(findsitemesure){
        try{
            await sitesMesure.updateOne({id: bodyVal.order_id}, {$set: {status: prod_json.status[1], modifiedAt: `${date.getDate()}/${date.getMonth() + 1}/${date.getFullYear()} ${date.getHours()}:${date.getMinutes()}`}})
            ctx.response.status = Status.OK
            return ctx.response.body = {
                SuccMsg: 'Statut changé avec succès'
            }
